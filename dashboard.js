...
async function showTaskByStage(projectId, stage) {
    ...
    try {
        ...
        const stageId = getStageIdByName(stage) || stage || null;
...
async function loadTaskDetailStageOptions(projectId, currentStageId) {
    ...
    let stages = [];
    if (projectId) {
        stages = await fetchOptions('/api/stages', {projectId: projectId});
    }
    ...
    if (currentStageId !== null && currentStageId !== undefined && currentStageId !== '') {
        ...
        if (!hasOption) {
            ...
            opt.value = val;
            opt.textContent = getStageNameById(currentStageId) || val;
...
function applyTaskToDetailModal(modalRoot, task) {
    ...
    try {
        syncFollowButtonState(task.id || modalRoot.dataset.taskId, modalRoot);
    } catch (e) {}
...
async function editTaskDetail(taskId, projectId) {
    ...
    try {
        ...
        try {
            syncFollowButtonState(taskId, modalRoot);
        } catch (e) {}
        ...
        try {
            ...
            console.warn('Failed to toggle controls for locked status', e);
        }
